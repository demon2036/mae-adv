...
if __name__ == "__main__":
    model = MaskedAutoencoderViT(img_size=32,
                                 patch_size=2, embed_dim=384, depth=12, num_heads=3,
                                 decoder_embed_dim=512, decoder_depth=1, decoder_num_heads=16,
                                 mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6)
                                 )
    print(model.state_dict().keys())

    model.load_state_dict(torch.load('checkpoints/mae-deit-s2-32-in1k-1600ep-d1-ema.pth'))

    dataset = CIFAR10('../data', train=True, transform=ToTensor())
    ...
    for data in dataloader:
        ...
        x_flat = einops.rearrange(x, 'b c (h d1) (w d2) -> b (h w) (d1 d2  c)', d1=2, d2=2)
