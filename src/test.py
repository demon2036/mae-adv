...
def train_and_evaluate(args):
  ...
  """
    ...
    if args.restore_type == 'orbax':
        ...
        state = checkpointer.restore(os.getcwd() + '/' + args.pretrained_model, item=ckpt)['model']

        # state = checkpointer.restore(os.getcwd() + '/' + args.pretrained_model, item=ckpt)['model']
        # params = state['params']
        # print(params['model'].keys())

    dataset = CIFAR10('../data', train=True, transform=PILToTensor())
    ...
    for data in dataloader:
        ...
        data_jax = jnp.array(np.array(x))

        data = einops.rearrange(x, 'b c h w ->b h w c')
        rngs, updates = state.split_rngs()
        rngs['random_masking']+=1
        print(rngs)
        metric = state.apply_fn({'params': state.params}, data_jax, rngs=rngs)
        #{'mse_loss': jnp.mean(loss), 'pred': pred, 'mask': mask}
        pred = metric['pred']
        mask = metric['mask']

        print(metric['mse_loss'])

        data_jax_flat = einops.rearrange(data_jax, 'b c (h d1) (w d2) -> b (h w) (d1 d2  c)', d1=2, d2=2)
        mask = jnp.astype(mask, jnp.int32)
        print(pred.shape, mask.shape)

        print(data_jax.max(), pred.max())

        out = mask[:, :, None] * data_jax_flat / 255 + (1 - mask[:, :, None]) * pred
        out = mask[:, :, None] * pred + (1 - mask[:, :, None]) * data_jax_flat / 255
