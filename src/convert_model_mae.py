...
def convert(params, checkpoint):
    ...
    decoder_pos_embed = params["model"]["decoder_pos_embed"]
    ...
    state_dict = {
        ...
        "decoder_pred.weight": params["model"]["decoder_pred"]['kernel'].transpose(1, 0),
        "decoder_pred.bias": params["model"]["decoder_pred"]["bias"],

        "decoder_embed.weight": params["model"]["decoder_embed"]['kernel'].transpose(1, 0),
        "decoder_embed.bias": params["model"]["decoder_embed"]["bias"],
        'mask_token': params["model"]['mask_token']
    ...
    state_dict["decoder_pos_embed"] = decoder_pos_embed
    ...
    if "decoder_norm" in params["model"]:
        print('use norm')
        state_dict["decoder_norm.weight"] = params["model"]["decoder_norm"]["scale"]
        state_dict["decoder_norm.bias"] = params["model"]["decoder_norm"]["bias"]
    ...
    for name, layer in params["model"].items():
        if not name.startswith("decoder_layer_"):
            continue
        print(len('decoder_layer_'))
        layer_idx = int(name[14:])

        if "q_norm" in layer["attn"]:
            print('use qk_norm')
            state_dict[f"decoder_blocks.{layer_idx}.attn.q_norm.weight"] = layer["attn"]["q_norm"]["scale"]
            state_dict[f"decoder_blocks.{layer_idx}.attn.q_norm.bias"] = layer["attn"]["q_norm"]["bias"]
            state_dict[f"decoder_blocks.{layer_idx}.attn.k_norm.weight"] = layer["attn"]["k_norm"]["scale"]
            state_dict[f"decoder_blocks.{layer_idx}.attn.k_norm.bias"] = layer["attn"]["k_norm"]["bias"]
        else:
            print('Do not use qk_norm')
        ...
        state_dict[f"decoder_blocks.{layer_idx}.attn.qkv.weight"] = qkv
        state_dict[f"decoder_blocks.{layer_idx}.attn.qkv.bias"] = jnp.concatenate(
            (
                layer["attn"]["wq"]["bias"].reshape(-1),
                layer["attn"]["wk"]["bias"].reshape(-1),
                layer["attn"]["wv"]["bias"].reshape(-1),
        ...
        state_dict[f"decoder_blocks.{layer_idx}.attn.proj.weight"] = wo.transpose(1, 0)
        state_dict[f"decoder_blocks.{layer_idx}.attn.proj.bias"] = layer["attn"]["wo"]["bias"]

        fc1 = layer["ff"]["w1"]["kernel"].transpose(1, 0)
        fc2 = layer["ff"]["w2"]["kernel"].transpose(1, 0)
        state_dict[f"decoder_blocks.{layer_idx}.mlp.fc1.weight"] = fc1
        state_dict[f"decoder_blocks.{layer_idx}.mlp.fc1.bias"] = layer["ff"]["w1"]["bias"]
        state_dict[f"decoder_blocks.{layer_idx}.mlp.fc2.weight"] = fc2
        state_dict[f"decoder_blocks.{layer_idx}.mlp.fc2.bias"] = layer["ff"]["w2"]["bias"]

        state_dict[f"decoder_blocks.{layer_idx}.norm1.weight"] = layer["norm1"]["scale"]
        state_dict[f"decoder_blocks.{layer_idx}.norm1.bias"] = layer["norm1"]["bias"]
        state_dict[f"decoder_blocks.{layer_idx}.norm2.weight"] = layer["norm2"]["scale"]
        state_dict[f"decoder_blocks.{layer_idx}.norm2.bias"] = layer["norm2"]["bias"]

        if "scale1" in layer:
            state_dict[f"decoder_blocks.{layer_idx}.ls1.gamma"] = layer["scale1"]
        if "scale2" in layer:
            state_dict[f"decoder_blocks.{layer_idx}.ls2.gamma"] = layer["scale2"]
